...
class gEMASimManager:
    ...
    def start_subprocess(self, id):
        process = Process(target=self.run_gem5_simulator, args=(id,), name=f"simulation_{id}")
        process.start()

    def run_gem5_simulator(self, id):
        print(f"Simulation ID: {id} PPID: {os.getppid()} PID: {os.getpid()}")
        board = self.root.configurator.generate_config(self.root.configs.get(f"config_{id}").get("config"))
